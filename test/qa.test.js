...
test('fileLabel / displayName', () => {
  assert.equal(fileLabel('colab-software.txt'), 'Colab Software');
  assert.equal(displayName('answers.txt', {}), 'My Answers');
  assert.equal(displayName('certn.txt', {}), 'Certn');
  assert.equal(displayName('certn.txt', { 'certn.txt': 'Certn Inc' }), 'Certn Inc');
});

test('nameToSlug: lowercases and replaces non-alphanumerics, trims dashes', () => {
  assert.equal(nameToSlug('CoLab Software'), 'colab-software');
