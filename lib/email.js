...
// When stripping removes everything (e.g. "Product Manager" → nothing), falls back to
// the raw role words so multi-role companies don't mix all emails together.
const LEVEL_WORDS = /^(senior|junior|lead|principal|staff|associate|sr|jr)$/i;
function roleBodyKeywords(role) {
  const stripped = extractRoleWords(role, BODY_ROLE_STRIP);
  if (stripped.length >= 2) return stripped.map(w => w.toLowerCase());
  // Fallback: raw role words excluding level qualifiers
  const raw = (role || '')
    .replace(/\(.*?\)/g, ' ')
    .replace(/[-–—|,\/&+]/g, ' ')
    .split(/\s+/)
    .map(w => w.trim())
    .filter(w => w.length > 2 && !LEVEL_WORDS.test(w));
  return raw.length >= 2 ? raw.map(w => w.toLowerCase()) : [];
}
...
function buildJobEmailQuery(companyTerm, roleTerm) {
  ...
  const roleClause = roleTerm
    ...
    : '';
  // Use phrases for noisy terms: raw "application" matches software apps,
  // raw "offer" matches subscription/pricing emails. Phrases are job-specific.
  const keywordClause = roleTerm
    ? ''
    : ' ("your application" OR "job application" OR "applied for" OR interview OR recruiter OR hiring OR "thank you for applying" OR "next steps" OR "job offer" OR "offer letter")';
