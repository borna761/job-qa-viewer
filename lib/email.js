...
// True pipeline order — earlier stage first — used only to decide whether a
// Gmail-derived classification should be allowed to overwrite Notion's stage.
// Deliberately separate from TRACKER_STAGE_RANK above: that array orders
// stages by "how worth searching for," not chronological progression (it
// puts Interviews before Applied), and reusing it here let a default-
// classified "Applied" email silently downgrade a genuine "Interviews" entry
// whenever classifyThread couldn't confidently detect interview language —
// e.g. a "thank you for your interview" reply or a reschedule/cancellation
// notice don't match its scheduled/invitation/confirmed/link pattern and fall
// through to the Applied default, even though the thread is clearly further
// along than a first application. classifyThread only ever returns Applied,
// Interviews, or Rejected, but all 6 stages appear here since Notion's
// existing stage (the overwrite target) can be any of them.
const STAGE_PROGRESSION_RANK = Object.fromEntries(
  ['Interested', 'Applied', 'Interviews', 'Stale', 'Turned Down', 'Rejected'].map((s, i) => [s, i])
);
