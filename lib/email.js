...
function buildJobEmailQuery(companyTerm, roleTerm) {
  ...
  const roleClause = roleTerm
    ...
    : '';
  // Scope keywords to subject so product/marketing emails (e.g. "Autodesk software offer")
  // don't match just because a job word appears in the body.
  const keywordClause = roleTerm
    ? ''
    : ' (subject:application OR subject:interview OR subject:recruiter OR subject:hiring OR subject:applying OR subject:"thank you for applying" OR subject:"next steps")';
