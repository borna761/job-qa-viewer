...
function buildJobEmailQuery(companyTerm, roleTerm) {
  ...
  // the company from the subject). fromTerm strips punctuation so "Stripe, Inc."
  // becomes "from:stripe" — a substring match in Gmail's from: operator.
  const fromTerm = companyTerm.toLowerCase().replace(/[^a-z0-9]/g, '');
...
async function enrichEmailDates(apps, access) {
  ...
  async function fetchDate(app) {
    ...
    if (app.stage === 'Rejected' || app.stage === 'Turned Down') {
      ...
      const hasActiveAtCompany = apps.some(a =>
        a !== app &&
