...
function buildJobEmailQuery(companyTerm, roleTerm, { strict = false } = {}) {
  // Match by subject (status update emails say "Update from Autodesk on…") OR by
  // sender domain (ATS confirmation emails from e.g. autodesk@myworkday.com omit
  // the company from the subject). Keep dots in fromTerm so "Booking.com" becomes
  // "from:booking.com" rather than "from:bookingcom" (which matches nothing).
