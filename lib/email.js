...
function buildJobEmailQuery(companyTerm, roleTerm) {
  // Role words are added as loose body-search terms so emails that mention the
  // specific role get prioritised by Gmail; keyword clause is always present so
  // generic ATS confirmation emails (which don't repeat the role name) are caught too.
  ...
  const roleClause = roleTerm
    ...
    : ' ' + JOB_KEYWORDS;
  return `subject:"${companyTerm}"${roleClause} ${GMAIL_EXCLUDE} newer_than:730d`;
}
