...
const ATS_DOMAINS = new Set([
  'lever', 'greenhouse', 'workday', 'myworkday', 'myworkdayjobs', 'ashbyhq', 'jobvite', 'breezy',
...
const rankFromOrder = order => Object.fromEntries(order.map((s, i) => [s, i]));

// Lower rank = more active/important (used to pick the "primary" entry per company for email enrichment)
const TRACKER_STAGE_RANK = rankFromOrder(
  ['Interviews', 'Applied', 'Interested', 'Stale', 'Turned Down', 'Rejected']
);

// Used only to decide whether a Gmail-derived classification should be allowed
// to overwrite Notion's stage. Deliberately separate from TRACKER_STAGE_RANK
// above: that array orders stages by "how worth searching for," not by how
// much confidence a Gmail signal should carry, and reusing it here let a
// default-classified "Applied" email silently downgrade a genuine
// "Interviews" entry whenever classifyThread couldn't confidently detect
// interview language — e.g. a "thank you for your interview" reply or a
// reschedule/cancellation notice don't match its scheduled/invitation/
// confirmed/link pattern and fall through to the Applied default, even
// though the thread is clearly further along than a first application.
//
// 'Stale' and 'Turned Down' are Notion-side buckets the user files
// applications into by hand, not stages Gmail ever classifies into — they
// rank just above 'Applied' (not above 'Interviews') so a confidently-matched
// 'Interviews' or 'Rejected' email can still revive one of them (real
// evidence the manual bucketing is out of date), while the weak 'Applied'
// default cannot touch them. classifyThread only ever returns Applied,
// Interviews, or Rejected, but all 6 stages appear here since Notion's
// existing stage (the overwrite target) can be any of them.
const STAGE_PROGRESSION_RANK = rankFromOrder(
  ['Interested', 'Applied', 'Stale', 'Turned Down', 'Interviews', 'Rejected']
);
...
function mergeGmailIntoNotion(notionApps, gmailApps) {
  ...
  for (const g of gmailApps) {
    ...
    if (!primary) continue;
    if ((STAGE_PROGRESSION_RANK[g.stage] ?? -1) > (STAGE_PROGRESSION_RANK[primary.stage] ?? -1))
...
// Strips generic job title words. ROLE_STRIP is used for Gmail query terms;
// BODY_ROLE_STRIP is used for post-fetch body keyword matching. Both strip the same
// set so neither searches for nor matches words that are too generic to be distinctive.
const ROLE_STRIP      = /\b(senior|junior|lead|principal|staff|associate|sr|jr|product|manager|owner|analyst|pm|po|tpm|tpo|cpo|vp|head|director|remote|canada|canadian|multiple|levels|available|contract|interim|part.?time|full.?time|solutions|platform|platforms|services)\b/gi;
// BODY_ROLE_STRIP intentionally matches ROLE_STRIP: level words (senior/lead/etc.) are too generic
// to use as body-matching keywords and would exclude unrelated emails containing those words.
// Generic corporate words (solutions/platform/services) are stripped for the same reason.
const BODY_ROLE_STRIP = /\b(senior|junior|lead|principal|staff|associate|sr|jr|product|manager|owner|analyst|pm|po|tpm|tpo|cpo|vp|head|director|remote|canada|canadian|multiple|levels|available|contract|interim|part.?time|full.?time|solutions|platform|platforms|services)\b/gi;
...
function roleSearchTerm(role) {
  ...
  return words.length >= 1 ? words.slice(0, 3).join(' ') : null;
}

// Keywords for post-fetch body filtering — strips the same generic words as ROLE_STRIP
// so level titles and common corporate nouns don't produce false positives.
...
// Job-related keyword clause — always included so ATS confirmation emails are found
// even when the role words don't appear in the email body.
const JOB_KEYWORDS = '("your application" OR "job application" OR "applied for" OR "for applying" OR "your interest" OR "votre candidature" OR interview OR recruiter OR hiring OR "thank you for applying" OR "next steps" OR "job offer" OR "offer letter")';

function buildJobEmailQuery(companyTerm, roleTerm, { strict = false } = {}) {
  // Match by subject (status update emails often say "Update from <Company> on…") OR by
  // sender domain (ATS confirmation emails from e.g. company@myworkday.com omit
  // the company from the subject). Keep dots in fromTerm so a company name containing
  // a dot still produces a matching from: clause (stripping dots yields a substring
  // that won't appear in the real sender address).
  const fromTerm = companyTerm.toLowerCase().replace(/[^a-z0-9.]/g, '').replace(/\.$/, '');
  const companyClause = `(subject:"${companyTerm}" OR from:${fromTerm})`;
  // strict=true omits JOB_KEYWORDS so only emails explicitly mentioning the role
  // are returned — used by enrichEmailDates to avoid cross-role date contamination
  // at companies with multiple open applications.
  // Role terms are wrapped in their own group so Gmail parses the OR correctly:
  // (("word1" "word2") OR JOB_KEYWORDS) rather than "word1" AND ("word2" OR JOB_KEYWORDS).
  const roleClause = roleTerm
    ? strict
      ? ' ' + roleTerm.split(' ').map(w => `"${w}"`).join(' ')
      : ' ((' + roleTerm.split(' ').map(w => `"${w}"`).join(' ') + ') OR ' + JOB_KEYWORDS + ')'
    : ' ' + JOB_KEYWORDS;
  return `${companyClause}${roleClause} ${GMAIL_EXCLUDE} newer_than:730d`;
}
...
// Returns true when an email clearly belongs to a specific role (all keywords match).
// Used to exclude emails that belong to a competing role at the same company.
function emailBelongsToRole(text, keywords) {
  if (!keywords.length) return false;
  const lower = text.toLowerCase();
  return keywords.every(w => lower.includes(w));
}
...
async function enrichEmailDates(apps, access) {
  ...
  async function fetchDate(app) {
    // For terminal-stage entries, skip when emails can't be safely attributed:
    // - no distinctive role term AND there's an active entry at the same company
    //   (company-only search would contaminate the active entry's date)
    // - has a distinctive role term that collides with an active entry's term
    if (app.stage === 'Rejected' || app.stage === 'Turned Down') {
      // Require 2+ distinctive words for collision detection: a single surviving word
      // (e.g. "Engineer" after stripping "Solutions" or "Platform") is too generic to
      // safely attribute emails to this specific role.
      const collisionWords = extractRoleWords(app.role, ROLE_STRIP);
      const thisRoleTerm = collisionWords.length >= 2 ? collisionWords.slice(0, 3).join(' ') : null;
      const sameCompany = (a) =>
        a !== app && a.company.toLowerCase() === app.company.toLowerCase() &&
        a.stage !== 'Rejected' && a.stage !== 'Turned Down';
      const hasActiveAtCompany = apps.some(sameCompany);
      if (!thisRoleTerm && hasActiveAtCompany) return;
      if (thisRoleTerm && apps.some(a => {
        if (!sameCompany(a)) return false;
        const aw = extractRoleWords(a.role, ROLE_STRIP);
        return aw.length >= 2 && aw.slice(0, 3).join(' ') === thisRoleTerm;
      })) return;
    }
    ...
    try {
      ...
      //    Use strict mode (no JOB_KEYWORDS fallback) so a generic job-keyword email
      //    from a different role at the same company can't overwrite this entry's date.
      if (roleTerm) {
...
module.exports = {
  ...
  buildJobEmailQuery, emailMatchesRole, emailBelongsToRole, enrichEmailDates,
