...
// Server-side counterpart to extension/popup.js's readJobPostingJsonLd —
// used when the real posting lives behind a cross-origin iframe the
// extension's content script can't read into directly (the extension only
// ever reports the iframe's src; this fetches and parses that page itself).
// Returns {role, company} or null when no JobPosting node has both.
function extractJobPostingRoleCompany(html) {
  const re = /<script[^>]+type=["']application\/ld\+json["'][^>]*>([\s\S]*?)<\/script>/gi;
  let m;
  while ((m = re.exec(html)) !== null) {
    let json;
    ...
    for (const node of nodes) {
      if (node && node['@type'] === 'JobPosting') {
        ...
        if (role && company) return { role: String(role), company: String(company) };
      }
    }
  }

  // Greenhouse's embed/job_app page (what a company careers page embeds via
  // "?gh_jid=..."): the application form is client-rendered with no
  // JobPosting ld+json at all, but the server always sets its <title> to
  // "Job Application for {role} at {company}".
  const titleMatch = html.match(/<title>\s*Job Application for\s+(.+?)\s+at\s+(.+?)\s*<\/title>/i);
  if (titleMatch) {
    const decode = (s) => s.replace(/&amp;/g, '&').replace(/&#(\d+);/g, (_, n) => String.fromCharCode(+n));
    return { role: decode(titleMatch[1]), company: decode(titleMatch[2]) };
  }
...
// Keep in sync with extension/shared.js's KNOWN_ATS_HOSTS (same list) — this
// one can't be literally shared with it: this is a server-side Node module,
// and the extension has no module system connecting it to this repo's lib/.
const EMBEDDABLE_ATS_HOSTS = ['ashbyhq.com', 'greenhouse.io', 'lever.co', 'myworkday.com', 'myworkdayjobs.com', 'icims.com'];
...
// Shared by fetchAndExtractBlocks and fetchAndExtractRoleCompany below —
// both need the identical fetch-with-UA-and-timeout, just to extract
// different things from the result. Returns '' if the fetch is blocked,
// fails, or times out.
...
// Fetch a URL server-side and extract {role, company} from its JobPosting
// JSON-LD — used to resolve the real role/company for a posting that lives
// behind a cross-origin iframe (see extension/popup.js's readJobPostingJsonLd
// and its embeddedJobUrl detection). Returns null if the fetch fails or no
// JobPosting node with both fields is found.
async function fetchAndExtractRoleCompany(targetUrl) {
  const html = await fetchHtml(targetUrl);
  if (!html) return null;
  return extractJobPostingRoleCompany(html);
}
